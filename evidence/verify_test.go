...
func makeLunaticEvidence(
	...
	require.NoError(t, err)
	byzantineValidators := conflictingVals.Copy().Validators
	...
	trustedBlockID := makeBlockID(trustedHeader.Hash(), 1000, []byte("partshash"))
	trustedVals, trustedPrivVals := types.RandValidatorSet(totalVals, defaultVotingPower)

	// Should be the same order between VoterSet.Voters and PrivValidators for Commit
	trustedPrivVals = orderPrivValsByValSet(t, trustedVals, trustedPrivVals)
